...
export function KanbanBoard({ deals, onMoveDeal, onDeleteDeal }: { 
  ...
  function handleDragEnd(event: DragEnd) {
    ...
    // The over id can be a stage ID or another deal ID
    let newStage = over.id as string;
    
    // If dropped over another card, get that card's stage
      ...
      <div className="flex gap-6 h-[70vh] overflow-x-auto pb-4 custom-scrollbar">
        {columns.map((col) => (
          <div key={col.id} className="flex-shrink-0 w-80 flex flex-col gap-4">
            <div className="flex items-center justify-between px-2">
               <div className="flex items-center gap-2">
                  <div className={cn(
                    ...
                    col.id === 'Won' ? 'bg-green-500' :
                    col.id === 'Lost' ? 'bg-red-500' :
                    'bg-primary'
                  )} />
                  <h3 className="font-black text-slate-900 uppercase text-[11px] tracking-[0.1em]">{col.id}</h3>
               </div>
            ...
            <div className="flex-1 bg-slate-50/50 rounded-[2rem] p-3 ring-1 ring-slate-100/50 overflow-y-auto">
               <SortableContext 
                id={col.id}
               ...
               >
                 ...
                 {col.deals.length === 0 && (
                   <div className="h-24 flex items-center justify-center text-[10px] font-bold text-slate-300 uppercase tracking-widest border-2 border-dashed border-slate-100 rounded-2xl">
                      Drop deals here
                   </div>
                 )}
               </SortableContext>
