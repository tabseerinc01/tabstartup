...
export default function DynamicSEOPage() {
  ...
  useEffect(() => {
    async function fetchSEOPageAndStartups() {
      if (!firestore || !slug) return;
      setIsLoading(true);
      try {
        // 1. Fetch SEO Page Definition
        const q = query(
        ...
        if (!snap.empty) {
          ...
          // Dynamically apply filters from the document
          if (data.filters?.industry) {
            constraints.push(where('industry', '==', data.filters.industry));
          }
          if (data.filters?.location) {
            constraints.push(where('location', '==', data.filters.location));
          }
          if (data.filters?.stage) {
            constraints.push(where('stage', '==', data.filters.stage));
          }
