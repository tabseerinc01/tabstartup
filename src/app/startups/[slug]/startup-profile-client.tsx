      ...
      try {
        ...
        if (!slugSnap.empty) {
          ...
          if (idSnap.exists()) {
            ...
            // REDIRECT if ID was used but SLUG exists
        ...
        if (ownerUid) {
          ...
          if (user?.uid) {
            // Check if investor interest exists
      ...
      <main className="flex-1 container mx-auto px-4 py-12">
        <div className="max-w-6xl mx-auto space-y-8">
          <div className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
             <Link href="/founders" className="hover:text-primary">Directory</Link>
             <span>/</span>
                ...
                <div className="flex flex-col md:flex-row justify-between items-end gap-6">
                  <div className="space-y-4 max-w-2xl">
                    <div className="flex items-center gap-3 flex-wrap">
                      <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight">{startup.name}</h1>
                      <Badge className="bg-white/20 text-white border-white/30 backdrop-blur-md px-4 py-1.5 rounded-xl text-sm">
                        {startup.stage} Stage
                      </Badge>
                    </div>
                  ...
                  <div className="flex flex-wrap gap-3 w-full md:w-auto">
                      ...
                      <>
                        {existingInterest ? (
                          ...
                          <Button 
                            ...
                            className="h-14 px-10 rounded-2xl bg-white text-primary hover:bg-white/90 shadow-xl" 
                        ...
                        <Button variant="outline" className="h-14 px-8 rounded-2xl border-white/30 text-slate-900 bg-white/10" asChild>
                    ...
                    <Button variant="secondary" className="rounded-2xl h-14 px-8 font-bold shadow-lg" asChild>
                      <Link href={`/founders/${startup.ownerUid}`}>Founder Info</Link>
                    </Button>
                  </div>
                </div>
              </div>
            ...
            <div className="px-8 md:px-12 py-10">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-8 pb-10 border-b">
                <div className="space-y-1">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-1.5"><Users className="h-3 w-3" /> Founder</p>
                  <Link href={`/founders/${startup.ownerUid}`} className="text-lg font-bold hover:text-primary transition-colors flex items-center gap-1">
                    {founder?.fullName || 'View Founder'} <ArrowRight className="h-4 w-4" />
                  </Link>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-1.5"><MapPin className="h-3 w-3" /> HQ</p>
                  <p className="text-lg font-bold">{startup.location || 'Remote'}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-1.5"><Tag className="h-3 w-3" /> Sector</p>
                  <p className="text-lg font-bold text-primary">{startup.industry}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-1.5"><HandCoins className="h-3 w-3" /> Need</p>
              ...
              <div className="mt-12 grid grid-cols-1 lg:grid-cols-3 gap-16">
                <div className="lg:col-span-2 space-y-16">
                  {startup.problem && (
                    <section className="space-y-4">
                      <h3 className="text-2xl font-bold flex items-center gap-2"><Lightbulb className="h-5 w-5 text-primary" /> The Problem</h3>
                      <p className="text-muted-foreground text-lg leading-relaxed whitespace-pre-wrap pl-4 border-l-2 border-primary/20">{startup.problem}</p>
                    </section>
                  )}
                  {startup.solution && (
                    <section className="space-y-4">
                      <h3 className="text-2xl font-bold flex items-center gap-2"><Rocket className="h-5 w-5 text-primary" /> The Solution</h3>
                      <p className="text-muted-foreground text-lg leading-relaxed whitespace-pre-wrap pl-4 border-l-2 border-primary/20">{startup.solution}</p>
                    </section>
                  )}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                    {startup.targetMarket && (
                      <section className="space-y-3">
                        <h4 className="text-xl font-bold flex items-center gap-2"><Users className="h-5 w-5 text-primary" /> Market</h4>
                        <p className="text-muted-foreground leading-relaxed">{startup.targetMarket}</p>
                      </section>
                    )}
                    {startup.businessModel && (
                      <section className="space-y-3">
                        <h4 className="text-xl font-bold flex items-center gap-2"><TrendingUp className="h-5 w-5 text-primary" /> Model</h4>
                        <p className="text-muted-foreground leading-relaxed">{startup.businessModel}</p>
                      </section>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-10">
                  <section className="bg-white p-8 rounded-[2.5rem] border shadow-xl sticky top-24 space-y-8">
                    <h3 className="text-xl font-bold border-b pb-4">Venture Insights</h3>
                    <div className="space-y-6">
                      <div className="space-y-1">
                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Funding</p>
                      ...
                      <div className="pt-6 border-t space-y-4">
                         {startup.website && (
                            <Button className="w-full h-14 rounded-2xl gap-2 font-bold" asChild>
                         ...
                         {startup.pitchDeckUrl && (
                            <Button variant="outline" className="w-full h-14 rounded-2xl gap-2 font-bold" asChild>
                              <a href={startup.pitchDeckUrl} target="_blank" rel="noopener noreferrer">
                                <FileText className="h-5 w-5" /> Pitch Deck <ExternalLink className="h-4 w-4 opacity-50" />
                              </a>
                            </Button>
                         )}
                      </div>
