      ...
      <main className="flex-1 container mx-auto px-4 py-12">
        <div className="max-w-4xl mx-auto space-y-8">
          <Card className="overflow-hidden border-none shadow-xl rounded-[2.5rem]">
            <div className="h-48 bg-gradient-to-r from-primary via-accent to-primary opacity-90" />
              ...
              <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <Badge variant="secondary" className="px-3 py-1 rounded-lg">{startup.stage}</Badge>
                  </div>
              ...
              <div className="mt-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
                <div className="lg:col-span-2 space-y-12">
                  <section>
                    <h3 className="text-2xl font-bold mb-4 flex items-center gap-2">
                      <Rocket className="h-6 w-6 text-primary" /> About the Venture
                    </h3>
                    <p className="text-muted-foreground text-lg leading-relaxed whitespace-pre-wrap">{startup.shortDescription}</p>
                  </section>
                  ...
                  {(startup.problem || startup.solution) && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t">
                      {startup.problem && (
                        <section>
                          <h4 className="text-lg font-bold mb-3">The Problem</h4>
                          <p className="text-muted-foreground leading-relaxed text-sm">{startup.problem}</p>
                        </section>
                      )}
                      {startup.solution && (
                        <section>
                          <h4 className="text-lg font-bold mb-3">The Solution</h4>
                          <p className="text-muted-foreground leading-relaxed text-sm">{startup.solution}</p>
                        </section>
                      )}
                    </div>
                  )}

                  {(startup.targetMarket || startup.businessModel) && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t">
                      {startup.targetMarket && (
                        <section>
                          <h4 className="text-lg font-bold mb-3">Target Market</h4>
                          <p className="text-muted-foreground leading-relaxed text-sm">{startup.targetMarket}</p>
                        </section>
                      )}
                    ...
                    <section className="bg-primary/5 p-8 rounded-3xl border border-primary/10">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="text-xl font-bold flex items-center gap-2">
                          <Lightbulb className="h-6 w-6 text-primary" /> Pitch Deck
                        </h3>
                ...
                <div className="space-y-10">
                  <section className="bg-white p-8 rounded-3xl border shadow-sm">
                    <h3 className="text-xl font-bold mb-6 flex items-center gap-2">Venture Details</h3>
                    <div className="space-y-6">
                      ...
                      {startup.website && (
                        <div className="flex items-center gap-3 text-muted-foreground">
                          ...
                          <a href={startup.website.startsWith('http') ? startup.website : `https://${startup.website}`} target="_blank" rel="noopener noreferrer" className="text-sm font-medium hover:underline text-primary truncate max-w-full">
                      ...
                      <div className="pt-6 border-t space-y-4">
                        ...
                        <div>
                          <p className="text-3xl font-bold text-primary">{startup.fundingNeed || 'TBD'}</p>
                          <Badge variant="outline" className="mt-2 font-bold border-primary/20 text-primary">Status: {startup.fundraisingStatus || 'Open'}</Badge>
                        </div>
                      </div>
                    </div>
                  </section>

                  <section className="bg-muted/30 p-8 rounded-3xl border border-muted-foreground/5">
